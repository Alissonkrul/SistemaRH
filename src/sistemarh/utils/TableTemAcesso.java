 ...
 * @author Alisson
 */
public class TableTemAcesso extends AbstractTableModel {
    ...
    public void seSistemas(List<Sistema> sistemas) {
        this.sistemas = sistemas;
        this.refreshTable();
    }

    public void addSis(List<Sistema> list) {
        for (Sistema sistema : list) {
    ...
    public List<Sistema> remove(int rows[]) {
        List<Sistema> deps = new ArrayList();
        for (int i = 0; i < rows.length; i++) {  
            deps.add(sistemas.remove(rows[i]-i));
        }
